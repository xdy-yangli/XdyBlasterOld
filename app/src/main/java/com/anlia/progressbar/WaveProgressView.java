...
public class WaveProgressView extends View {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        secondWaveColor = typedArray.getColor(R.styleable.WaveProgressView_second_wave_color, ContextCompat.getColor(context, R.color.light));
        bgColor = typedArray.getColor(R.styleable.WaveProgressView_wave_bg_color, Color.LTGRAY);
        typedArray.recycle();
...
//              percent = interpolatedTime * progressNum / maxNum;
                percent = progressNum / maxNum;
                if (onAnimationListener != null) {
                    if (textView != null) {
                        textView.setText(onAnimationListener.howToChangeText(interpolatedTime, progressNum, maxNum));
                    }
                    onAnimationListener.howTiChangeProgressColor(wavePaint, interpolatedTime, progressNum, maxNum);
                }
            }
...
//     * @param time        动画持续时间
     */
    public void setProgressWave(float progressNum) {
        this.progressNum = progressNum;
        percent = progressNum / maxNum;
        if (progressNum > 60)
            waveColor = ContextCompat.getColor(mContext, R.color.colorGreen);
        else if (progressNum > 30)
            waveColor = ContextCompat.getColor(mContext, R.color.yellow_light);
        else
            waveColor = ContextCompat.getColor(mContext, R.color.colorRed);
    ...
    public void setProgressNum(float progressNum, int time) {
        ...
        if (time != 0) {
            ...
            if (onAnimationListener != null) {
                if (textView != null) {
                    textView.setText(onAnimationListener.howToChangeText(1.0f, progressNum, maxNum));
                }
